...
resource "aws_subnet" "blue_public_subnet" {
  ...
  vpc_id                  = "${aws_vpc.vpc.id}"
  # cidr_block              = "${cidrsubnet(aws_vpc.vpc.cidr_block, 8, count.index)}"
   cidr_block              = "${element(var.public_cidr_block, 0)}"
  availability_zone       = "${data.aws_availability_zones.available.names[count.index]}"
  map_public_ip_on_launch = false

  tags = {
    name        = "blue-${var.app_name}-${var.environment}-public-subnet-${count.index}"
    environment = "${var.environment}"
    managed_by  = "${var.managed_by}"
  }
}
...
resource "aws_subnet" "green_public_subnet" {
  ...
  vpc_id                  = "${aws_vpc.vpc.id}"
  # cidr_block              = "${cidrsubnet(aws_vpc.vpc.cidr_block, 8, count.index)}"
   cidr_block              = "${element(var.public_cidr_block, 1)}"
  availability_zone       = "${data.aws_availability_zones.available.names[count.index]}"
  map_public_ip_on_launch = false

  tags = {
    name        = "green-${var.app_name}-${var.environment}-public-subnet-${count.index}"
    environment = "${var.environment}"
    managed_by  = "${var.managed_by}"
  }
}
